...
// COMPUTE SHADERS
// ===========================
const char surface_find_compute_shader[] = R"(#version 310 es
precision highp float;
...
int get_index_of_position(in ivec3 position) {
    ivec3 num_work = ivec3(gl_NumWorkGroups.xyz);
    return int(position.x + position.y * num_work.y + position.z * (num_work.z * num_work.z));
}
...
const char surface_triangularize_compute_shader[] = R"(#version 310 es
precision highp float;
...
void test_triangle_x(int index, in ivec3 curr_pos, in vec4 current_point) {
    ...
    if (vertices[t1_v1].w != 0.0 && vertices[t1_v2].w != 0.0) { // Early out
        uint start_vert = atomicAdd(mesh_vertices_count, 3u);
        mesh_vertices[start_vert] = current_point;
        mesh_vertices[start_vert+1u] = vertices[t1_v1];
        mesh_vertices[start_vert+2u] = vertices[t1_v2];
    }
    ...
    if (vertices[t2_v1].w != 0.0 && vertices[t2_v2].w != 0.0) { // Early out
        uint start_vert = atomicAdd(mesh_vertices_count, 3u);
        mesh_vertices[start_vert] = current_point;
        mesh_vertices[start_vert+1u] = vertices[t2_v1];
        mesh_vertices[start_vert+2u] = vertices[t2_v2];
    }
}
