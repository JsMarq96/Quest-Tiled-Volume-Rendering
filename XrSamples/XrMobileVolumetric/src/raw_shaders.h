...
vec3 mrm() {
    ...
    for(; i < MAX_ITERATIONS; i++) {
        ...
        float depth = textureLod(u_volume_map, sample_pos / 2.0 + 0.5, float(curr_mipmap_level)).r;
        if (depth > 0.05) { // There is a block
            //return vec3(1.0, 0.0, 0.0);
            if (curr_mipmap_level == 0u) {
                return vec3(1.0);
            }
            return vec3(1.0, 0.0, 0.0);
            curr_mipmap_level--;
            ...
            if (prev_dist < dist && !in_the_same_area(sample_pos, prev_sample_pos, float(curr_mipmap_level+1u))) {
                curr_mipmap_level++;
            } else {
                dist += get_distance(float(curr_mipmap_level));
            }
        }
...
void main() {
    ...
    vec3 ray_dir = normalize(ray_origin - u_camera_eye_local);
   //o_frag_color = texture(u_albedo_map, gl_FragCoord.xy / vec2(NOISE_TEX_WIDTH));
   //o_frag_color = render_volume(); //*
